...
func TestToken(t *testing.T) {
	p := newParser(strings.NewReader(parserXml))
	...
	for {
		...
		if tt == startType {
			if p.string(m[0]) != parserXmlStrings[j] { t.Fatal("wrong start tag name") }
			j++
			for i := 1; i + 1 < len(m); i, j = i + 2, j + 2 {
				if p.string(m[i]) != parserXmlStrings[j] { t.Fatal("wrong key") }
				if p.string(m[i + 1]) != parserXmlStrings[j + 1] { t.Fatal("wrong value") }
			}

			if len(m) % 2 == 0 {
				if a, b := p.string(m[0]), parserXmlStrings[j]; a != b {
					t.Fatalf("wrong end name, m = %v, j = %v, a = %v, b = %v", m, j, a, b)
				}
				j++
			}
		} else {
			if p.string(m[0]) != parserXmlStrings[j] { t.Fatal("wrong mark string") }
			j++
		}
